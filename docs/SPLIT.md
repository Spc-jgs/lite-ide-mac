...
> 状态：2026-09-21 做完（提交 37c588a → 75cfde3 → b68a769 → 53ce2bf，外加一步瘦身）。
> 和设计稿不一样的地方：标签右键那一项在单标签组里**不出现**而不是灰掉（`Tabs.svelte` 的规矩）；
> 入口包增量实测 +5.5 KB 不是 +2 KB（第 6 节）；光标行列要在编辑器拿到焦点时补报一次（4.3）。
...
- 上下分屏、三组、嵌套
- ~~拖标签到另一组~~ —— 2026-09-21 做了（`shell/tab-drag.ts`，pointer 事件自己做，按需加载）
- 分屏时的空态（不存在：空组自动收起）
