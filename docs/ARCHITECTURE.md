...
**二、把「我以为如此」写成运行时自检**（未做）。这个应用里有一批只活在
脑子里的不变量：log 句柄开了必关、标签的 `handle` 和 `mode` 必须同步、
`dirty` 与 `draft` 必须同真同假、会话恢复出来的标签数等于存进去的数。
形态是 `invariant(cond, msg)`，不成立时**写日志但不打断用户**。
`stashed` 那个 bug 正是这个形状 —— 它当时不满足的是
「快照写回去不该改变 mode」。

**三、把预算变成每次启动都量的数**（未做）。issue #10 那条内存超标是手工
`vmmap` 才发现的。形态是启动完成时往日志写一行 `boot=NNNms rss=NNNMB tabs=N`，
再配个脚本从日志拉最近几次做对比。「慢慢变胖」是最容易在没人注意时发生的一类，
它需要的不是告警，是**能回看的一串数**。
