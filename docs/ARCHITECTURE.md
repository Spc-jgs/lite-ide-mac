...
| 7 | M4 上 LSP（含 Java） | **不上 LSP，也不上 tree-sitter**：跳转靠 CM6 已有的 Lezer 树 + import 解析 | jdtls 启动 5–10s、常驻 1GB+，与"轻量"直接冲突。而 tree-sitter 也不需要 —— CM6 为高亮本来就把文档解析好了，再挂一套 wasm parser 是同一份代码解析两遍，且每种语言几百 KB，撞入口包 160KB 的红线。2026-09-09 落地，见下方「跳转做到哪一层」 |

### 跳转做到哪一层（修正 7 的落地形态）

按**精度**分层，不按范围分。只做前几层，因为只有它们不需要类型推断就能确定：

| 层 | 靠什么 | 准不准 |
|---|---|---|
| 零、Java 的局部变量 / 字段 / `q.成员`（2026-09-23） | 单文件符号表（`java-scope.ts`）+ 接收者**写出来的**声明类型所在的文件 | 准：推不出就不画，见下 |
...
`repo.findById()` 要知道 `repo` 是什么类型。零层在单文件里建一张按作用域嵌套的符号表
（同一张表也给字段使用处上紫色，见 `java-semantic.ts`），查到 `repo` 声明成
`OrderRepository`，再用第二层的办法找到那个文件、读出它的成员表。边界：

- **类型只认源码里写出来的**：字段 / 参数 / 局部变量的声明类型、`new Foo()`、`(Foo) x`、
  大写的类型名本身。`var`、链式调用的返回值、泛型实参一概不推 —— 推一半比不推更危险。
- **只跳一步、不追继承**：那个类自己没声明（Spring Data 的 `findById` 在 jar 里的父接口上）就不画。
- **重载按实参个数筛**，剩不下唯一一个就不画（和 `findIn` 撞两份认怂同一条）。
- **推不出 ≠ 不管**：接收者类型推不出时零层返回「不画」，而不是让它落到第一层按名字找 ——
  否则 `order.getItems()` 会跳到本文件里一个同名方法，还亮着下划线（这层上线之前就是这样）。
- 读别的文件是异步的，而下划线必须同步判定（⌘hover 每动一格问一次）：成员表放缓存，
  第一次 hover 到没读过的类时先不画，读回来在鼠标原地重问一次补上（`java-peek.ts`）。

Java 专用的这些代码跟着 Java 语言包懒加载，经 CM6 的 `languageData` 交给 `jump.ts` ——
`jump.ts` 不引它们，别的语言和入口包不为它买单。

日志视图里的堆栈帧（`at com.a.B.m(B.java:42)`）是同一个依据的另一处用法：包路径 + 括号里的
文件名拼成后缀、去文件索引里坐实，唯一命中才画成链接（`logview/stack-frame.ts`）。

实现分两个文件：`src/lib/editor/jump.ts`（纯函数，`tests/jump.test.ts` + `java-jump.test.ts`）和
...
**入口包 160KB 的红线（2026-09-21 从 150 放宽）只管 57ms 那一段，占启动的 14%。** 再压一半省 30ms，而前面
