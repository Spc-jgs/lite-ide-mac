...
它造一个中性的示例项目（orderflow，假的 Java 订单服务 + 33 万行日志 + 两个分支的
历史），起真 `.app`，用 AX 摆出四个画面，`screencapture` 之后缩到 1600 宽放进
`docs/screenshots/`。README 引用的就是这四个文件名。手拍的图没人知道当时开的是什么、
窗口多大，界面一改就对不上；脚本一跑就是同一批画面。会占用键盘十几秒，拍完自己收
拾（示例项目在 `/tmp`，拍图用的那份草稿从草稿目录删掉）。
...
| `pnpm build` | 确认前端能构建 |
| 入口包体积门禁 | 超过 **160 KB** 就失败，超过 145 KB 先告警 —— 见下 |
