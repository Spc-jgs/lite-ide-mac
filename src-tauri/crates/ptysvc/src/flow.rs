...
mod tests {
    ...
    fn 接到真_pty_上要把读出来的量按在水位附近() {
         ...
         */
        // **死线 90 秒，和 `lib.rs` 的 `PROMPT_BUDGET` 同一个理由**（issue #30）：
        // 这是一条*活性前提*，不是被测的东西 —— 等多久都不影响后面那 600ms
        // 的测量，而这台机器上 `zsh -l` 是双峰的（稳态 0.08 秒，冷的那一下
        // 实测到过 60.4 秒）。给前提留窄窗口是间歇红最常见的来源，
        // 而**一个会误报的测试，过一阵就没人当真了**。
        // 快的时候它照样立刻往下走，90 秒一分不花。
        let 死线 = Instant::now() + Duration::from_secs(90);
        while total.load(Ordering::Relaxed) < 8192 && Instant::now() < 死线 {
            std::thread::sleep(Duration::from_millis(20));
        }
        assert!(
            total.load(Ordering::Relaxed) >= 8192,
            "90 秒内 `yes` 都没跑起来（只读到 {} 字节）—— 这台机器上的登录 shell 太慢，\
             或者 write_input 没写进去",
