...
（**当前 142,131 B = 138.8 KiB（2026-09-21 晚），告警线 145、红线 160**；CI 算的是 `size / 1024` 取整。这两条线是 2026-09-21 从 138/150 放宽的：清单上能搬去懒 chunk 的都搬完了（下面两段），剩下的是接线；入口包只占启动的 14%（JOURNAL 09-18）。**放宽不等于不数**：这个数字每轮照旧重量、照旧记在这儿，涨了要说得出是谁。
